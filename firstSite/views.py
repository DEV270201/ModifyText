from django.http import HttpResponse
#for rendering the html templates
from django.shortcuts import render
import re

def home(request):
    print("in the home page")
    # return render(request,"home.html").
    #we can also pass the parameters
    return render(request,"home.html")

# def contact(request):
#     #if you want to switch to some other page of the same website
#     return HttpResponse("<h1>Contact Page</h1><br><a href='image'> Go to image page </a>")

# def image(request):
# 
#     return HttpResponse('<img src="data:image/jpeg;base64," alt="myimage">')

#removing the punctuations
def RemovePunctuations(text):
    text = re.sub(r'!|@|(|)|-|[|]|{|}|;|:|<|>|/|\.|#|\$|%|\^|&|\*|_|-|\'|\"', "" , text)
    print(text)
    return text

def RemoveNewLine(text):
 temp=""
 for i in text:
        if(i!='\n' and i!='\r'):
            temp = temp+i
 return temp

def RemoveExtraSpace(text):
    temp=""
    for index,i in enumerate(text):
        if(not(text[index] == " " and text[index+1] == " ")):
            print(text[index])
            temp = temp + i
    return temp


def Upper(text):
    temp=""
    for i in text:
        temp = temp + i.upper()
    return temp


def CharCount(text):
    return len(text)


def analyze(request):
    #it is used to get the data sent using get request
    # textRecieved = request.GET.get('text', "defaultValue")
    # removepunc = request.GET.get('removepunc' , "off") #default value of the checkbox will be off
    # removeextraspace = request.GET.get('removeextraspace' , "off") #default value of the checkbox will be off
    # removenewline = request.GET.get('removenewline' , "off") #default value of the checkbox will be off
    # charcount = request.GET.get('charcount' , "off") #default value of the checkbox will be off
    # upper = request.GET.get('upper' , "off") #default value of the checkbox will be off

    #if you dont want to expose the data, you can use post request
    print("POST : " , request.POST)
    textRecieved = request.POST.get('text', "defaultValue")
    removepunc = request.POST.get('removepunc' , "off") #default value of the checkbox will be off
    removeextraspace = request.POST.get('removeextraspace' , "off") #default value of the checkbox will be off
    removenewline = request.POST.get('removenewline' , "off") #default value of the checkbox will be off
    charcount = request.POST.get('charcount' , "off") #default value of the checkbox will be off
    upper = request.POST.get('upper' , "off") #default value of the checkbox will be off
    track = False
    global count
    count = 0

    #if the user has checked the removepunc then it will show on
    if(removepunc == "on"):
        textRecieved = RemovePunctuations(textRecieved)
        track = True
    if(removenewline == "on"):
        textRecieved = RemoveNewLine(textRecieved)
        track = True

    if(removeextraspace== "on"):
        textRecieved = RemoveExtraSpace(textRecieved)
        track = True

    if(charcount == "on"):
        count  = CharCount(textRecieved)
        track = True

    if(upper == "on"):
        textRecieved = Upper(textRecieved)
        track = True

    
    if(track):
        return render(request,"analyze.html",{"analyzedText" : textRecieved , "count" : count})
    else:
        return HttpResponse("Sorry, error occurred")

    # print("text : ", textRecieved)
    # return HttpResponse("remove the punctuation")










# def capFirst(request):
#     return HttpResponse("make the first letter capital")

# def removeSpace(request):
#     return HttpResponse("remove the spaces")

# def removeNewLine(request):
#     return HttpResponse("remove the new lines")

# def charCount(request):
#     return HttpResponse("count the characters")